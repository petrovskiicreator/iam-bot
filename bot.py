...
async def send_challenge_reminder():
    ...
    # Дни челленджа, которые не требуют записи (автозакрываются при открытии)
    ...
    for u in users:
        try:
            ...
            if ch_day <= 0:
                continue  # не начинал челлендж — не беспокоим

            # Текущий день челленджа (0-based)
            current_day_idx = ch_day % 21
            day_type = CHA[current_day_idx]

            if day_type in AUTO_COMPLETE_TYPES:
                continue  # автозакрывается при открытии приложения — не напоминаем

            # Проверяем, есть ли запись нужного типа сегодня
            arr_map = {"vision": data.get("vis") or [], "diary": data.get("diary") or [], "gratitude": data.get("grat") or []}
            arr = arr_map.get(day_type, [])
            if any(e.get("date", "")[:10] == today for e in arr):
                count_skipped += 1
                continue  # уже выполнил задание дня

            day_num = current_day_idx + 1
            type_label = {"vision": "визуализацию ✨", "diary": "запись в дневнике 📝", "gratitude": "благодарности 🙏"}.get(day_type, "задание")
            text = (
            ...
            await bot.send_message(
                u["user_id"], text,
                reply_markup=open_app_kb("Выполнить задание 🔥"),
                parse_mode=ParseMode.HTML
            )
            count_sent += 1
            await asyncio.sleep(0.05)
